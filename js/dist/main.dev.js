...
Snake.prototype.init = function () {
  ...
  snakeBody2.next = null; //给蛇添加一条属性，用来表示蛇走的方向
...
Snake.prototype.getNextPos = function () {
  ...
  this.head.x / sw + this.direction.x, this.head.y / sh + this.direction.y]; //下个点是自己，代表撞到了自己，游戏结束
...
Snake.prototype.strategies = {
  move: function move(format) {
    //这个参数用于决定要不要删除最后一个方块（蛇尾）。当传了这个参数后就表示要做的事情是吃
    //创建新身体（在旧蛇头的位置）
    var newBody = new Square(this.head.x / sw, this.head.y / sh, 'snakeBody'); //更新链表的关系
    ...
    newBody.create(); //创建一个新蛇头(蛇头下一个要走到的点nextPos)

    var newHead = new Square(this.head.x / sw + this.direction.x, this.head.y / sh + this.direction.y, 'snakeHead'); //更新链表的关系

    newHead.next = newBody;
    newHead.last = null;
    newBody.last = newHead;
    newHead.viewContent.style.transform = 'rotate(' + this.direction.rotate + 'deg)';
    newHead.create(); //蛇身上的每一个方块的坐标也要更新 //unshift
...
Game.prototype.over = function () {
  clearInterval(this.timer);
  alert('你的得分为：' + this.score); //游戏回到最初始的状态
